...
class PageForm extends Form
{
	public function initialize(PageParser $page, $options)
	{
		...
		$content_title = new Text('static_content_title', $attrs);
		$content_title->setLabel("Статичний заголовок:");
		$content_head = new TextArea('static_content_head_', $attrs);
		$content_head->setLabel("Блок html в шапці");
		$content_title2 = new Text('static_content_content_title', $attrs);
		$content_title2->setLabel("H2:");
		$content = $this->getWYSWIGField('static_content_content', $attrs);
		$content->setLabel("Статичний контент:");
		$content_body = new TextArea('static_content_body_', $attrs);
		$content_body->setLabel("Блок html в тілі");
		$content_footer = new TextArea('static_content_footer_', $attrs);
	...
	{
		if (\Phalcon\Text::startsWith($name,"static_content_")){
			$field = str_replace("static_content_","", $name );
	...
	public function bind(array $data, $entity, $whitelist = null)
	{
		...
		foreach (array_keys($data) as $name) {
			if (\Phalcon\Text::startsWith($name,"static_content_")){
				$field = str_replace("static_content_","", $name);
				$this->_entity->content->{$field} = $data[$name];
			}
		}
